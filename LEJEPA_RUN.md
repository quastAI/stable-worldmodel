...
python scripts/train/lejepa.py \
    ...
    loader.batch_size=256 program_constants.lambda=1.0e-2
...
| `program_constants.lambda` | 1.0e-2 | `loss = λ·sigreg + (1-λ)·align`, **in the paper's units** — `loss.sigreg.kwargs.pool_views: true` is what makes that so. The paper's λ×ρ grid (Fig. 6 / App. H.6) at ρ=0.9 puts all four candidates in the good region — R²(h→z) 0.981 / 0.989 / 0.993 / 0.992 for 1e-3 / 5e-3 / 1e-2 / 5e-2, separated only by orthogonality error 0.461 / 0.210 / 0.142 / **0.062**. On that table alone 5e-2 wins, and it was the default until the s3072 50-epoch run. **Three measurements moved it to 1e-2**, none available to the paper's grid: `fit/align_loss` *rose* 0.09→0.13 through the hold while `sigreg_loss` fell and `fit/trace_cov` was already flat at 10k steps (so this is not the floor growing); held-out alignment finished at **2.93×** its floor against the paper's converged 0.976×; and 24% of the embedding was style (`style_sensitivity` 0.244). Style dimensions cost the full `2σ²` under alignment rather than `2(1-ρ)σ²`, so raising alignment's relative weight is the term that should squeeze them out. The paper's 5e-2 does not transfer because its calibration never had this profile's independently-resampled style between the two views (`resample_style_within_pair: true`). Going lower risks collapse, not graceful degradation — `trace_cov` starts near 1.4 against n=10. **Do not go below 5e-3 without evidence.** |
...
python scripts/identifiability/run_metrics.py \
    ...
    program_constants.lambda=1.0e-2
```
...
| 11 | `λ = 3e-3` was read off Fig. 6's 2-D grid, and the repo's per-view SIGReg made it **1.5e-3 in the paper's units** — ~30× below the grid's best setting at ρ=0.9, and only ~15× above the λ < 1e-4 band Fig. 6 shows fails at every ρ. Wrong side to err on, given `trace_cov` starts near 1.4 against n=10 | `pool_views: true` puts λ in the paper's units. Set to `5e-2` (Fig. 6's best orthogonality at ρ=0.9), then to **`1e-2`** after the s3072 50-epoch run measured SIGReg winning against alignment — see the knobs table above |
